    ...
    <script>
        ...
        function exportToExcel() {
            ...
            try {
                ...
                const colWidths = headers.map((_, index) => {
                    ...
                    if (index === 4) return { wch: 12 };  // قيمة الخصم
                    if (index === 5) return { wch: 12 };  // الراتب قبل
                    if (index === 6) return { wch: 12 };  // الراتب بعد
                    if (index === 7) return { wch: 8 };   // النسبة
                    if (index === 8) return { wch: 12 };  // التاريخ
                ...
                XLSX.utils.book_append_sheet(wb, ws, "خصومات الموظفين");
                XLSX.writeFile(wb, `خصومات_الموظفين_${new Date().toISOString().slice(0, 10)}.xlsx`);
        ...
        async function exportToPDF() {
            ...
            try {
                ...
                title.textContent = 'تقرير خصومات جميع الموظفين';
                ...
                const options = {
                    margin: [15, 15, 20, 15],
                    filename: `خصومات_الموظفين_${new Date().toISOString().slice(0, 10)}.pdf`,
                    image: {
                        type: 'jpeg',
                        quality: 0.98
                    },
