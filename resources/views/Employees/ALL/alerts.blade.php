        ...
        <div class="rounded-xl overflow-hidden mb-6 alerts-shadow alerts-gradient-bg">
            <div class="alerts-header-gradient bg-red-800 px-6 py-4 flex items-center justify-between text-white">
                ...
                <div class="flex items-center space-x-3 rtl:space-x-reverse">
                    <span class="alerts-badge px-3 py-1 rounded-full text-sm font-medium">{{ count($alerts) }} إنذار</span>
                ...
                <table class="table min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">عنوان الإنذار</th>
                            <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">السبب</th>
                            <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">تاريخ الإرسال</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="alerts-row hover:bg-red-50 transition-colors">
                                ...
                                <td class="alerts-cell px-6 py-4 whitespace-nowrap text-center">
                                    @if($alert->employee->project && $alert->employee->project->manager)
                                        <div class="flex flex-col items-center space-y-1">
                                            ...
                                            <span class="text-sm text-gray-700">{{ $alert->employee->project->manager->name }}</span>
                                        </div>
                                    @else
                                        <span class="text-gray-400">-</span>
                                    @endif
                                ...
                                <td class="alerts-cell px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex flex-col items-center">
                                        <span class="text-sm font-medium text-gray-900">{{ $alert->created_at->format('Y-m-d') }}</span>
                                        <span class="text-xs text-gray-500">{{ $alert->created_at->locale('ar')->diffForHumans() }}</span>
                                    </div>
                                </td>
                            </tr>
                        @empty
