            ...
            <div class="bg-teal-50 hover:bg-teal-100 border-l-4 border-teal-500 rounded-lg px-6 py-4 flex items-center justify-between text-black transition-all duration-300">
                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">عرض جميع الخصومات المسجلة</p>
                    </div>
                </div>
                <div class="flex items-center space-x-3 rtl:space-x-reverse">
                    <span class="bg-teal-100 hover:bg-teal-200 px-3 py-1 rounded-full text-sm font-medium text-gray-800 transition-all">
                        {{ count($advancesDeductions) }} خصم
                    </span>
                    <span class="bg-teal-100 hover:bg-teal-200 px-3 py-1 rounded-full text-sm font-medium text-gray-800 transition-all">
                        {{ number_format($advancesDeductions->sum('amount')) }} ر.س إجمالي الخصومات
                    </span>
                        ...
                        <div class="w-full max-w-md">
                            <div class="search-container px-4 py-2 flex items-center gap-3">
                                ...
                                <input
                                    type="text"
                                    id="liveSearch"
                                    placeholder="ابحث في الموظفين أو المبالغ..."
                        ...
                        <tr class="advance-row hover:bg-teal-50 transition-colors">
                            ...
                            <td class="advance-cell px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex flex-col items-center space-y-1">
                                    <a href="{{ route('employees.advances_deductions', $deduction->employee->id) }}"
                                       class="employee-link text-sm font-medium text-gray-900 hover:text-teal-600">
                            ...
                            <td class="advance-cell px-6 py-4 whitespace-nowrap text-center">
                                <span class="text-sm text-gray-700">{{ number_format($deduction->advance->amount ?? 0) }} ر.س</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            liveSearch.addEventListener('input', function() {
                ...
                tableRows.forEach(row => {
                    ...
                    // Get employee name (2nd column) and amount (3rd column)
                    ...
                    // Check if search term matches employee name OR amount
                    ...
                    if (matchesEmployee || matchesAmount) {
                        row.style.display = '';
                    } else {
            ...
            function exportToExcel() {
                ...
                try {
                    ...
                    const colWidths = headers.map((_, index) => {
                        ...
                        if (index === 3) return { wch: 15 }; // تاريخ الخصم
                        if (index === 4) return { wch: 15 }; // السلفة
                    ...
                    XLSX.writeFile(wb, `خصومات_السلف_${new Date().toISOString().slice(0, 10)}.xlsx`);
            ...
            async function exportToPDF() {
                ...
                try {
                    ...
                    title.textContent = 'تقرير خصومات جميع السلف';
                    ...
                    const options = {
                        margin: [15, 15, 20, 15],
                        filename: `خصومات_جميع_السلف_${new Date().toISOString().slice(0, 10)}.pdf`,
                        image: {
                            type: 'jpeg',
                            quality: 0.98
                        },
