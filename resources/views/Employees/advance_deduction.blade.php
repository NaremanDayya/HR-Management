                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse($employee->advanceDeductions as $index => $deduction)
                            <tr class="advance-row hover:bg-blue-50 transition-colors">
                                <td class="advance-cell px-6 py-4 text-center text-sm text-gray-500">{{ $index + 1 }}</td>
                                <td class="advance-cell px-6 py-4 text-center font-bold text-teal-700">{{ number_format($deduction->amount) }} ر.س</td>
                                <td class="advance-cell px-6 py-4 text-center">{{ $deduction->deducted_at->format('Y-m-d') }}</td>
                                <td class="advance-cell px-6 py-4 text-center">
                                    <span class="text-xs text-gray-700">{{ number_format($deduction->advance->amount) }} ر.س</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
