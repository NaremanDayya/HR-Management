        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            liveSearch.addEventListener('input', function() {
                ...
                tableRows.forEach(row => {
                    ...
                    // Get amount (2nd column) and reason (7th column)
            ...
            function exportToExcel() {
                ...
                try {
                    ...
                    const colWidths = headers.map((_, index) => {
                        ...
                        if (index === 2) return { wch: 15 }; // النسبة من الراتب
                        if (index === 3) return { wch: 15 }; // مدير المشروع
                        if (index === 4) return { wch: 15 }; // المشرف
                        if (index === 5) return { wch: 15 }; // منطقة العمل
                        if (index === 6) return { wch: 25 }; // السبب
                        if (index === 7) return { wch: 12 }; // تاريخ الطلب
                        if (index === 8) return { wch: 12 }; // تاريخ المعالجة
                        if (index === 9) return { wch: 15 }; // مدة الاستجابة
                        if (index === 10) return { wch: 12 }; // الحالة
                    ...
                    XLSX.writeFile(wb, `سلف_الموظف_${"{{ $employee->user->name }}"}_${new Date().toISOString().slice(0, 10)}.xlsx`);
