    ...
    @else
        <div id="chat-footer" x-data="{
            ...
            setupEmojiPicker() {
                ...
                if (picker) {
                    picker.addEventListener('emoji-click', event => {
                        ...
                        this.body = this.body.substring(0, startPos) + emoji + this.body.substring(endPos);
            ...
            <section
                    ...
                    @if (count($files) > 0)
                        <section x-data="{
                            ...
                            removeUpload(filename) {
                                $wire.removeUpload(this.type, filename);
                            },
                            handleFileSelect(event, count) {
                                if (event.target.files.length) {
                                    const files = event.target.files;
                                    const remainingSlots = this.MAXFILES - count;

                                    if (files.length > remainingSlots) {
                                        $dispatch('wirechat-toast', {
                                            type: 'warning',
                                            message: `You can only upload ${remainingSlots} more files`
                                        });
                                        event.target.value = '';
                                        return;
                                    }

                                    $wire.uploadMultiple('files', Array.from(event.target.files), () => {
                                        event.target.value = '';
                                    });
                                }
                            }
                        }" @files-updated.window="$nextTick(() => {})"
                            class="flex overflow-x-scroll ms-overflow-style-none items-center w-full col-span-12 py-2 gap-5"
                            style="scrollbar-width: none; -ms-overflow-style: none;">
                ...
                <form x-init="window.addEventListener('emoji-click', function(event) {
                    body += event.detail.unicode;
                });" x-data="{
                    body: $wire.entangle('body'),
                    sendMessage() {
                        // Allow sending if there are files/media OR if there's a non-empty message
                        if (($wire.media.length > 0) || ($wire.files.length > 0) || (body && body.trim().length > 0)) {
                            $wire.sendMessage();
                    ...
                    class="flex items-center col-span-12 w-full gap-2 gap-5">
                    @csrf
                                ...
                                @if (config('wirechat.allow_file_attachments', true))
                                    <label wire:loading.class="cursor-progress" x-data="attachments('files')"
                                        ...
                                        <input type="file" wire:model="files" multiple
                                            accept="{{ Helper::formattedFileMimesForAcceptAttribute() }}"
                                            class="sr-only">
                                        <div
                                ...
                                @if (config('wirechat.allow_media_attachments', true))
                                    <label wire:loading.class="cursor-progress" x-data="attachments('media')"
                                        ...
                                        <input type="file" wire:model="media" multiple
                                            accept="{{ Helper::formattedMediaMimesForAcceptAttribute() }}"
                                            class="sr-only">
                    ...
                    <div @class(['flex gap-2 sm:px-2 w-full'])>
                        <textarea x-ref="body" x-model="body" x-data="{
                            get body() { return $wire.body || '' },
                            set body(value) { $wire.set('body', value) }
                        }" @focus-input-field.window="$el.focus()"
                            autocomplete="off" x-model='body' x-ref="body" wire:loading.delay.longest.attr="disabled"
                            wire:target="sendMessage" id="chat-input-field" autofocus type="text" name="message"
                            ...
                            @keyup.enter.prevent="$event.shiftKey ? null : ((($wire.media.length > 0 || $wire.files.length > 0) || (body && body?.trim().length > 0)) ? $wire.sendMessage() : null)"
                            class="w-full disabled:cursor-progress resize-none h-auto max-h-20  sm:max-h-72 flex grow border-0 outline-0 focus:border-0 focus:ring-0  hover:ring-0 rounded-lg   dark:text-white bg-none dark:bg-inherit  focus:outline-hidden   "></textarea>
                    ...
                    <div x-cloak @class(['w-[5%] justify-end min-w-max  items-center gap-2 '])>
                        ...
                        <button
                            ...
                            <svg class="w-7 h-7 dark:text-red-700" xmlns="http://www.w3.org/2000/svg" width="36"
            ...
            @push('scripts')
                <script>
                    Alpine.data('attachments', (type = "media") => ({
                        ...
                        // In your attachments Alpine component
                        uploadFiles(files) {
                            this.isUploading = true;
                            this.progress = 0;

                            console.log('Starting upload of', files.length, 'files'); // Debug
                            ...
                            files.forEach((file, index) => {
                                $wire.upload(
                                    ...
                                    file,
                                    () => {
                                        console.log('Upload complete for', file.name); // Debug
                                        this.progress = 100;
                                    ...
                                    (event) => {
                                        ...
                                        console.log('Upload progress:', this.progress); // Debug
                                    }
                                );
                            });
                        }
